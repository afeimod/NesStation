...
void fault_handler(int sn, siginfo_t * si, void *segfault_ctx)
{
        ...
        // NesStation dcfix6: last-resort self-healing for faults inside memory we
        // manage (vmem arena / nvmem buffers). If we reach this point the fault was
        // not claimed by any of the handlers above, which on some OEM ROMs happens
        // because the SIGSEGV handler chain was disturbed mid-run. Restore the page
        // protection and keep emulating instead of killing the process.
        if (addrspace::selfHealManagedFault(si->si_addr))
                return;
