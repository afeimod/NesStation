...
bool region_lock(void *start, size_t len)
{
        ...
        // dcfix8 SAFE_MEM: emulator data pages are never write-protected;
        // dirty-page polling recovers block/texture invalidations instead
        // (TexCache.cpp / blockmanager.cpp). Keeping this a no-op guarantees
        // SIGSEGV faults on emulator memory cannot happen at all, so a
        // clobbered fault-handler chain can no longer kill the process.
        return true;
#else
...
// NesStation dcfix6: non-fatal variant used during vmem init. A failure there
// must trigger the clean nvmem fallback instead of dying (or worse, leaving a
// half-initialized vmem state behind).
static bool region_unlock_safe(void *start, size_t len)
{
        size_t inpage = (uintptr_t)start & PAGE_MASK;
        return mprotect((u8*)start - inpage, len + inpage, PROT_READ | PROT_WRITE) == 0;
}
...
bool init(void **vmem_base_addr, void **sh4rcb_addr, size_t ramSize)
{
        ...
        *vmem_base_addr = (void*)(ptrint + sizeof(Sh4RCB));
        ...
        // NesStation dcfix6: if this mprotect fails the vmem state would be
        // unusable (SH4 context stays PROT_NONE) — tear everything down and let
        // the caller fall back to the nvmem path instead of running half-broken.
        if (!region_unlock_safe(sh4rcb_base_ptr, sizeof(Sh4RCB) - fpcb_size))
        {
                ERROR_LOG(VMEM, "vmem init: failed to unlock SH4 context - falling back to nvmem");
...
#ifdef NESSTATION_DC_SAFE_MEM
        // dcfix8 SAFE_MEM: keep FPCB pages read-write (no PROT_NONE traps).
        // addrspace::bm_reset() fills the whole jump table eagerly instead.
        return;
#else
