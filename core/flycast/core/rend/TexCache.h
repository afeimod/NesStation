...
// NesStation dcfix8 (SAFE_MEM): sweep the texture-locked VRAM pages once per
// frame and invalidate exactly like the SIGSEGV path would. Only meaningful
// when NESSTATION_DC_SAFE_MEM is enabled (otherwise page-protection faults do
// the job); safe no-op when no texture locks exist.
