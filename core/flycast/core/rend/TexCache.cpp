...
// NesStation dcfix8 SAFE_MEM texture invalidation.
//
// With page protection disabled (posix_vmem region_lock is a no-op), the
// classic "write fault -> VramLockedWrite" invalidation can never fire.
// Instead the emulation thread polls once per frame: every page that
// currently carries registered texture locks is compared against a shadow
// copy, and changed pages go through VramLockedWriteOffset() — the exact
// code the fault path uses (invalidate textures, drop the locks). Only
// locked pages are compared, so the cost is a few hundred 4K memcmps per
// frame at worst. Writes through VRAM mirrors (0x06000000) hit the same
// physical pages and are picked up too, which the fault path could miss.
static u8* s_vramShadow;
static std::vector<u32> s_vramPollPages;

static u8* vramPollHostBase() {
        if (addrspace::virtmemEnabled())
                return addrspace::ram_base + 0x04000000;
        return &vram[0];
}

void VramPollDirtyPages()
{
        if (VramLocks == nullptr)
                return;
        if (s_vramShadow == nullptr)
        {
                s_vramShadow = (u8*)malloc(VRAM_SIZE);
                if (s_vramShadow == nullptr)
                        return;
                memcpy(s_vramShadow, vramPollHostBase(), VRAM_SIZE);
        }

        // Snapshot the pages that currently carry texture locks. The render
        // thread mutates the lists under vramlist_lock, so read under the
        // same lock; release before taking it again in the invalidate path.
        s_vramPollPages.clear();
        {
                std::lock_guard<std::mutex> lockguard(vramlist_lock);
                const u32 maxPages = VRAM_SIZE / PAGE_SIZE;
                for (u32 i = 0; i < maxPages; i++)
                {
                        std::vector<vram_block*>& list = VramLocks[i];
                        for (vram_block* lock : list)
                        {
                                if (lock != nullptr)
                                {
                                        s_vramPollPages.push_back(i);
                                        break;
                                }
                        }
                }
        ...
        u8* vramHost = vramPollHostBase();
        for (u32 page : s_vramPollPages)
        {
                const size_t offset = (size_t)page * PAGE_SIZE;
                if (memcmp(s_vramShadow + offset, vramHost + offset, PAGE_SIZE) == 0)
                        continue;
                memcpy(s_vramShadow + offset, vramHost + offset, PAGE_SIZE);
                // Same invalidation the SIGSEGV path performs (locks its own mutex).
