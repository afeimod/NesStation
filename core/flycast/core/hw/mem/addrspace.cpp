...
void bm_reset()
{
        ...
        if (ram_base)
        {
#ifdef NESSTATION_DC_SAFE_MEM
                // dcfix8 SAFE_MEM: no PROT_NONE traps on FPCB pages — fill the
                // whole jump table eagerly with the default entries instead.
...
bool reserve()
{
        ...
        if (!virtmem::init((void**)&ram_base, (void**)&p_sh4rcb, RAM_SIZE_MAX + VRAM_SIZE_MAX + ARAM_SIZE_MAX + elan::ERAM_SIZE_MAX))
        {
                // NesStation dcfix6: honor the init failure and make sure the
                // fallback (nvmem) path is engaged cleanly. initMappings() checks
                // ram_base and allocates regular heap buffers when it is null.
                ram_base = nullptr;
                p_sh4rcb = nullptr;
                WARN_LOG(VMEM, "vmem init FAILED - falling back to nvmem (slow-path) memory mode");
        }
        return true;
}
...
// NesStation dcfix6: last-resort SIGSEGV self-healing (see addrspace.h).
//
// The dynarec / texture cache rely on write-protecting pages we own (RAM code
// protection, VRAM texture invalidation, FPCB on-demand pages, fastmem
// rewrites) and on the process-wide SIGSEGV handler to unlock them on demand.
// If that handler chain is disturbed by another library (GL drivers, OEM
// "crash monitoring" libs on some OEM ROMs, crash SDKs) or a protection
// survives a core restart, one of those writes turns into a fatal
// SEGV_ACCERR inside addrspace::write32/read32 called from JIT code
// ([anon:.bss]) — the exact tombstone seen on Redmi socrates / HyperOS.
//
// This hook runs AFTER all the specific handlers declined the fault: if the
// address is inside memory we manage, simply restore write access and keep
// emulating. Worst case we miss one invalidation (a one-frame visual glitch)
// instead of crashing the whole app.
bool selfHealManagedFault(void* faultAddr)
{
        static u8* healedPages[64];
        static unsigned healedCount = 0;
        static unsigned totalHeals = 0;

        u8* page = (u8*)((uintptr_t)faultAddr & ~(uintptr_t)(PAGE_SIZE - 1));
        bool managed = false;

        if (ram_base != nullptr && p_sh4rcb != nullptr)
        {
                // vmem arena: Sh4RCB (incl. fpcb) sits below ram_base, mapped
                // mirrors span up to 0x21000000 (0x20000000 + writable ARAM 8MB).
                u8* lo = (u8*)p_sh4rcb;
                u8* hi = ram_base + 0x21000000;
                managed = page >= lo && page < hi;
        }
        else if (p_sh4rcb != nullptr)
        {
                // nvmem fallback buffers (heap): sh4rcb + ram + vram + aica + elan
                auto inRange = [&page](u8* base, size_t size) {
                        return base != nullptr && page >= base && page < base + size;
                };
                managed = inRange((u8*)p_sh4rcb, sizeof(Sh4RCB))
                        || inRange(&mem_b[0], RAM_SIZE)
                        || inRange(&vram[0], VRAM_SIZE)
                        || inRange(&aica::aica_ram[0], ARAM_SIZE)
                        || inRange(elan::RAM, elan::ERAM_SIZE);
        }

        if (!managed)
                return false;

        // Repeat offender: this page was already healed once and faults again —
        // this is NOT one of our protections (genuine wild access), let it die.
        for (unsigned i = 0; i < healedCount; i++)
        {
                if (healedPages[i] == page)
                {
                        ERROR_LOG(COMMON, "[dcfix6] self-heal repeat fault on page %p - giving up", (void*)page);
                        return false;
                }
        }

        if (mprotect(page, PAGE_SIZE, PROT_READ | PROT_WRITE) != 0)
                // Not a protection problem (e.g. genuinely unmapped) - not ours.
                return false;

        if (healedCount < 64)
                healedPages[healedCount++] = page;
        totalHeals++;
        if (totalHeals <= 20)
                ERROR_LOG(COMMON, "[dcfix6] self-healed write-protected page %p (fault addr %p, heal #%u)", (void*)page, faultAddr, totalHeals);
...
bool selfHealManagedFault(void* faultAddr)
{
        // No SIGSEGV-based self-healing on Windows builds.
        return false;
}
#endif
