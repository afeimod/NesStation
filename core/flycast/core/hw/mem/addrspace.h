...
// NesStation dcfix6: last-resort SIGSEGV self-healing.
// Returns true if the fault address lies inside memory we manage ourselves
// (the vmem arena, or the nvmem fallback buffers) and write access has been
// restored. Used by the fault handler when none of the specific handlers
// claimed the fault: rather than dying, restore the page protection and keep
// emulating. This covers situations where the process-wide SIGSEGV handler
// chain is disturbed by other libraries (GL drivers, OEM monitoring libs) or
// where a stale page protection survived a core restart.
