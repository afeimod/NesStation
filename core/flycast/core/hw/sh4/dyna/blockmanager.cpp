...
void bm_DiscardBlock(RuntimeBlockInfo* block)
{
        ...
        // Remove from jump table — under NESSTATION_DC_SAFE_MEM the FPCA
        // entry may already have been overwritten (e.g. by bm_ResetTempCache
        // clearing stale blocks without calling Discard).  Treat a mismatch
        // as a warning instead of a fatal crash so the emulator can keep
        // running.
        if ((void*)bm_GetCode(block_ptr->addr) == CC_RW2RX((void*)block_ptr->code))
        {
                FPCA(block_ptr->addr) = ngen_FailedToFindBlock;
        }
        else
        {
                ERROR_LOG(DYNAREC, "bm_DiscardBlock: FPCA mismatch for block @%08x "
                         "(FPCA=%p, code=%p) — skipping FPCA reset",
                         block_ptr->addr,
                         (void*)bm_GetCode(block_ptr->addr),
                         (void*)CC_RW2RX((void*)block_ptr->code));
        }
...
void bm_ResetTempCache(bool full)
{
        if (!full)
        {
                for (const auto& block : all_temp_blocks)
                {
                        ...
                        // Discard() must run to remove the block from
                        // blocks_per_page[]. Without it, bm_PollDirtyRamPages
                        // finds the stale entry next frame and bm_DiscardBlock
                        // crashes because the FPCA no longer matches.
                        block->Discard();
                }
        }
...
// NesStation dcfix8 SAFE_MEM block invalidation.
//
// RAM pages holding compiled blocks are no longer write-protected (see
// posix_vmem.cpp), so bm_RamWriteAccess() can no longer be triggered by a
// SIGSEGV fault. Instead the emulation thread polls once per frame: every
// page that carries registered blocks is compared against a shadow copy and
// bm_RamWriteAccess() runs for the changed ones (discard stale blocks).
// unprotected_pages[] is re-armed afterwards so future blocks keep watching
// the page — the fault path leaves it permanently unprotected instead.
static u8* s_ramShadow;

void bm_PollDirtyRamPages()
{
        if (blocks_per_page == nullptr)
                return;
        u8* ramHost = addrspace::virtmemEnabled()
                ? addrspace::ram_base + 0x0C000000
                : &mem_b[0];
        if (s_ramShadow == nullptr)
        {
                s_ramShadow = (u8*)malloc(RAM_SIZE);
                if (s_ramShadow == nullptr)
                        return;
                memcpy(s_ramShadow, ramHost, RAM_SIZE);
        }

        for (u32 p = 0; p < RAM_SIZE / PAGE_SIZE; p++)
        {
                if (blocks_per_page[p].empty())
                        continue;
                const u32 offset = p * PAGE_SIZE;
                if (memcmp(s_ramShadow + offset, ramHost + offset, PAGE_SIZE) == 0)
                        continue;
                memcpy(s_ramShadow + offset, ramHost + offset, PAGE_SIZE);
                bm_RamWriteAccess(offset);
                // Re-arm: without faults this flag would otherwise block future
                // registrations and the page would go unwatched forever.
                unprotected_pages[p] = false;
        }
}
#endif
