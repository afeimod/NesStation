...
void retro_run()
{
        ...
        // NesStation dcfix8 SAFE_MEM: recover block/texture invalidations by
        // polling the registered pages once per frame instead of relying on
        // SIGSEGV page-protection faults (no fault -> nothing can crash the
        // process even when the fault-handler chain gets clobbered).
        VramPollDirtyPages();
        bm_PollDirtyRamPages();
#endif
